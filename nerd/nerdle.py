...
def istrue(chrs):
    try:
        return eval(''.join(chrs))
    except:
        return False


class Clue:
    __CLUE_STR_REQS = "cluestr must be `DDDDDDDD CCCCCCCC` | C∈{B,G,Y}, L∈[0-9+-*/=], cnt('=')=1"
    __DSET = set(string.digits).union(set("+-*/="))

    @classmethod
    def ident(cls):
        res = cls("1111=111 BBBBBBBB")
        res.includes['1'] = [0, INF]
        for s in res.possbyspot:
            s.add('1')
        return res

    def __init__(self, cluestr: str) -> None:
        if len(cluestr) != 17 or cluestr[8] != ' ':
            raise ValueError(self.__CLUE_STR_REQS)
        lets, cols = cluestr.split()
        if ((not all(
                map(functools.partial(operator.contains, Clue.__DSET), lets)))
                or
            (not all(
                map(functools.partial(operator.contains, set("BGY")), cols)))):
            raise ValueError(self.__CLUE_STR_REQS)
        generalpos = set(Clue.__DSET)
        self.includes = {k: [0, INF] for k in generalpos}
        for k in set(lets):
    ...
    def __matches(self, ostr: typing.List[str]):
        octr = Counter(ostr)
        try:
            return (len(ostr) == 8 and eval(''.join(ostr)) \
                    and all(
                        map(functools.partial(operator.contains, Clue.__DSET),
                            ostr))
    ...
    def matches(self):
        for ostr in itertools.product(*self.possbyspot):
            if self.__matches(ostr):
                yield ostr
...
def interact():
    ...
    print("Each line should first have the digits guessed,"
    ...
    print("Hint: 1+7*9=64")
    print(
        "To repeat the same guesses you've already entered prior, enter `*`.")
...
def maxsingletest():
    ...
    for guess in Clue.ident().matches():
        clues = Counter()
        for targ in Clue.ident().matches():
